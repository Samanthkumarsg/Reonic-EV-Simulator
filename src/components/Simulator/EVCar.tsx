...
const EVCar: React.FC<{
    isCharging?: boolean;
    batteryLevel?: number;
    carIndex: number;
}> = ({ isCharging = false, batteryLevel = 50, carIndex }) => {
    ...
    return (
        <div className="group relative shrink-0 ">
            <div className="border flex items-center justify-center flex-col relative bg-gray-50 px-3 py-2 rounded-lg shadow-sm transition-transform duration-300 group-hover:scale-105 ">

                <div className=" w-12 p-0.5 bg-white rounded-full shadow-sm">
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                            className={`h-full transition-all duration-1000 rounded-full ${isCharging ? 'animate-pulse' : ''
                ...
                {/* Car SVG */}
                <div className="w-8 h-8 text-gray-800 transition-colors group-hover:text-blue-900 flex items-center justify-between">
              ...
              S6.7,10,7.5,10S9,10.7,9,11.5S8.3,13,7.5,13z M16.5,13c-0.8,0-1.5-0.7-1.5-1.5s0.7-1.5,1.5-1.5s1.5,0.7,1.5,1.5S17.3,13,16.5,13z"/>
                    ...
                    <h1 className="text-sm font-medium "> {carIndex + 1}</h1>
                </div>
                ...
                {isCharging && (
                    <div className="absolute -right-2 top-1/2 -translate-y-1/2">
                        <div className="relative">
                            <svg className="w-5 h-5 text-yellow-600 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="absolute opacity-0 group-hover:opacity-100 transition-opacity duration-200 -top-24 left-1/2 -translate-x-1/2 pointer-events-none z-50">
                <div className="relative bg-gray-900 text-white px-3 py-2 rounded-lg shadow-lg text-sm whitespace-nowrap">
                    <div className="flex flex-col gap-1">
                        <div>Car id: #{carIndex + 1}</div>
